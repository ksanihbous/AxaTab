--==========================================================
--  AxaHub - TAB 7 "CHAT PUBLIK"
--  Loader Chat Relay (WK CHAT PUBLIK HG + IC + STT + Subtitle + Filter UI)
--==========================================================

------------------- GLOBAL STT STUB (AMAN DIPANGGIL DULUAN) -------------------
_G.AxaChatRelay_STTQueue = _G.AxaChatRelay_STTQueue or {}
local AxaChatRelay_STTQueueRef = _G.AxaChatRelay_STTQueue

local function AxaChatRelay_EarlyEnqueueSTT(...)
    table.insert(AxaChatRelay_STTQueueRef, {...})
end

-- Kalau belum ada fungsi global, pakai stub queue dulu
if type(_G.AxaChatRelay_ReceiveSTT) ~= "function" then
    _G.AxaChatRelay_ReceiveSTT = AxaChatRelay_EarlyEnqueueSTT
end

------------------- LOADER TAB 7: CHAT PUBLIK -------------------
local function AxaHub_StartChatPublik()
    -- Biar nggak jalan dua kali
    if _G.AxaChatRelay_Started then
        warn("[AxaHub] TAB CHAT PUBLIK sudah aktif, skip reload.")
        return
    end
    _G.AxaChatRelay_Started = true

    --==========================================================
    --  Axa Chat Relay - Public Chat + System Info + STT + Discord
    --==========================================================
    ...
    ------------------- STT QUEUE (supaya aman kalau dipanggil terlalu cepat) -------------------
    -- Catatan: queue global & stub sudah dibuat di Core di atas.
    _G.AxaChatRelay_STTQueue = _G.AxaChatRelay_STTQueue or {}
    ...
    local WEBHOOK_URLS = {
        ...
        --"https://discord.com/api/webhooks/1440543878603345920/006xVzHfgRLzJ1lupG6Sizklcq_DeODRVHDOnzBRXEnKVp5WSATrKDC8QUVuWHW2MeTf", -- tambahan
    }
    ...
    -- Filter subtitle hanya player terdekat
    local NEARBY_CAPTION_ONLY = false
    local NEARBY_MAX_DISTANCE = 17 -- studs, ±0,28–0,3 m / stud
    ...
    local function createFilterUI()
        ...
        local screenGui = Instance.new("ScreenGui")
        screenGui.Name = "AxaChatFilterUI"
        screenGui.ResetOnSpawn = false
        screenGui.IgnoreGuiInset = true
        screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
        screenGui.Parent = playerGui
        ...
        makeCheckButton("All Chat", FILTER_ALLCHAT, function(state)
            FILTER_ALLCHAT = state
        end)

        makeCheckButton("System Info", FILTER_SYSTEMINFO, function(state)
            FILTER_SYSTEMINFO = state
        end)

        makeCheckButton("Special UserID / koneksi", FILTER_SPECIALCHAT, function(state)
        ...
        makeCheckButton("Subtitle hanya player terdekat", NEARBY_CAPTION_ONLY, function(state)
        ...
        local function updateDock()
            if dockOpen then
                dockFrame.Size = UDim2.new(0, 260, 0, 210)
    ...
    local function startChatRelay()
        ...
        print("[Axa Chat Relay] Aktif: relay chat -> Discord webhook + history file + SPECIAL shiny ✨"
            .. " + avatar thumbnail square 420x420 + per-user Special Mention (+ fallback Axa)"
            .. " + Chat Khusus Mirethos/Kaelvorn (pink) + YAML fields"
            .. " + UI Filter Chat (All/System/Special/System Special/Chat Khusus + Subtitle Near)"
...
--==========================================================
--  CARA PANGGIL DARI CORE (TAB 7)
--==========================================================
-- Di handler tab-mu, misal:
-- if selectedTabIndex == 7 then
--     AxaHub_StartChatPublik()
-- end